...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if load_from_clip == False:
            ...
            # print(self.positional_embedding_res.shape)
    ...
    def forward(self, image, text_long, class_ids, rank):
        ...
        # Compute class similarity mask
        # Shape: (batch_size, all_batch_size)
        class_mask = (class_ids.unsqueeze(1) == class_ids_all.unsqueeze(0)).float()  # 1 for same class, 0 for different classes

        # Apply softmax over similarities with class mask
        sim_i2tl_softmax = F.softmax(sim_i2tl, dim=1) * class_mask
        sim_tl2i_softmax = F.softmax(sim_tl2i, dim=1) * class_mask

        # Normalize to ensure proper distribution
        sim_i2tl_softmax = sim_i2tl_softmax / (sim_i2tl_softmax.sum(dim=1, keepdim=True) + 1e-8)
        sim_tl2i_softmax = sim_tl2i_softmax / (sim_tl2i_softmax.sum(dim=1, keepdim=True) + 1e-8)

        # Compute loss
        bs = image.size(0)
        targets = torch.arange(rank * bs, rank * bs + bs, device=image.device)

        # Class-aware contrastive loss
        loss_i2tl = -torch.log(sim_i2tl_softmax[torch.arange(bs), targets] + 1e-8).mean()
        loss_tl2i = -torch.log(sim_tl2i_softmax[torch.arange(bs), targets] + 1e-8).mean()

        loss_itcl = (loss_i2tl + loss_tl2i) / 2
        return loss_itcl
...
def add_missing_keys(model, state_dict):
    ...
    for key in model_dict:
        if key not in state_dict:
            ...
            print(f"[INFO] Resizing key '{key}' from shape {state_dict[key].shape} to {model_dict[key].shape}")
            state_dict[key] = model_dict[key]  # 크기가 맞지 않는 경우 모델 기본값 사용
    return state_dict
...
def build_model(state_dict: dict, load_from_clip: bool):
    ...
    print(model.positional_embedding_res.shape)
    state_dict = add_missing_keys(model, state_dict)

    # Positional Embedding 크기 조정 (필요시)
    if "positional_embedding" in state_dict and state_dict["positional_embedding"].shape != model.state_dict()["positional_embedding"].shape:
        print(f"[INFO] Resizing 'positional_embedding' from shape {state_dict['positional_embedding'].shape} to {model.state_dict()['positional_embedding'].shape}")
        state_dict["positional_embedding"] = torch.nn.functional.interpolate(
            state_dict["positional_embedding"].unsqueeze(0), 
            size=model.state_dict()["positional_embedding"].shape[0], 
            mode='linear',
            align_corners=False
        ).squeeze(0)
